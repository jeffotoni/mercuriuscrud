...
// var collections = make(map[string]string)

// inserindo perguntas na base de dados
func PerguntaCreate(ctx *context.Context) {
			...
			if string(byteJson) != "" {
				...
				if err != nil {
					...
					if Tp.Ppr_dtcadastro == "" {
						...
						msgJson = `{"status":"error","msg":"A coluna Ppr_dtcadastro obrigatoria!"}`

					} else if Tp.Ppr_dtaltera == "" { // campo obrigatorio string

						msgJson = `{"status":"error","msg":"A coluna Ppr_dtaltera obrigatoria!"}`
						...
						if err != nil {
							...
							msgJson = `{"status":"error","msg":"A collection pode esta errada ou sua condicao error: ` + err.Error() + `"}`
							...
							if exist {
								...
								ID, err := repo.Insert(collection, Tp)
